...
Seed sample preorder data locally:

```bash
npm run seed
```

This reads `prisma/seed-data/preorders.json` and inserts 15 new preorder records. The JSON file does not include ids; ids are generated by the database.

You can also seed data through the API, including on a deployed/live server:
...
```txt
POST https://your-live-domain.com/preorder/seed
```

This uses the same JSON file and inserts the same 15 sample preorder records into the connected production/deployed database.
...
### Seed Sample Preorders Through API

```txt
POST /preorder/seed
```

This inserts the 15 sample records from `prisma/seed-data/preorders.json`. It does not require a request body.

Use this when you want to seed a deployed/live database without running terminal commands on the server.

Note: every request inserts a fresh copy of the sample rows.
